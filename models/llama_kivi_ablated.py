...
class LlamaAttention_KVCOMP_ABLATED(nn.Module):
...
class LlamaFlashAttention_KVCOMP_ABLATED(LlamaAttention_KVCOMP_ABLATED):
...
class LlamaDecoderLayer_KVCOMP_ABLATED(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = (
            ...
            else LlamaFlashAttention_KVCOMP_ABLATED(config=config)
        )
...
class LlamaModel_KVCOMP_ABLATED(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer_KVCOMP_ABLATED(config) for _ in range(config.num_hidden_layers)])
...
class LlamaForCausalLM_KVCOMP_ABLATED(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModel_KVCOMP_ABLATED(config)
        self.vocab_size = config.vocab_size
