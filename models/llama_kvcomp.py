...
_CONFIG_FOR_DOC = "LlamaConfig2"

# time_taken2 = [0 for i in range(4)]
...
def dequantize(quantized: torch.Tensor, scale: float, offset: float):
    odtype = quantized.dtype
    val = quantized * scale + offset
    return val.to(odtype)
...
# @timer(3)
def decompress_with_nvcomp(compressed_arr, shape, dtype):
    decoded = codec.decode(compressed_arr)  # Avoid unnecessary numpy conversion
    return torch.as_tensor(decoded, dtype=dtype, device="cuda").view(shape)  # Direct to FP16
...
def compress(tensor: torch.Tensor, num_bits: int = 2):
    quantized, scale, shift = quantize(tensor, num_bits)
    compressed_arr, original_shape = compress_with_nvcomp(quantized)
    obj = CompObject(
...
def decompress(compressed_obj: CompObject):
    decompressed_arr = decompress_with_nvcomp(compressed_obj.compressed, compressed_obj.shape, compressed_obj.dtype)
    dequant_arr = dequantize(decompressed_arr, compressed_obj.scale, compressed_obj.shift)
    dequant_arr = dequant_arr.view(compressed_obj.shape)
    return dequant_arr
    

class LlamaAttention_KVCOMP(nn.Module):
...
class LlamaFlashAttention_KVCOMP(LlamaAttention_KVCOMP):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            value_scale = past_key_value[6]
            value_mn = past_key_value[7]
            # value_states_shape, key_states_shape, kss, kso = past_key_value[8]
            
            
            if value_states_quant is not None:
                value_states_quant = decompress(value_states_quant)
            if key_states_quant_trans is not None:
                key_states_quant_trans = decompress(key_states_quant_trans)
            # if key_states_full is not None:
            #     key_states_full = decompress(key_states_full)
        ...
        # if key_states_full is not None:
        #     key_states_full = compress(key_states_full)
        if key_states_quant_trans is not None:
            key_states_quant_trans = compress(key_states_quant_trans)
        if value_states_quant is not None:
            value_states_quant = compress(value_states_quant)
                                    
        # nv_comp_states = (value_states_shape, key_states_full_shape, kso, kss)
...
class LlamaDecoderLayer_KVCOMP(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = (
            # LlamaAttention(config=config)
            LlamaAttention_Compression_Flash(config=config)
            if not getattr(config, "use_flash", True)
            else LlamaFlashAttention_KVCOMP(config=config)
        )
        setattr(config, "use_flash", True)
...
class LlamaModel_KVCOMP(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer_KVCOMP(config) for _ in range(config.num_hidden_layers)])
...
class LlamaForCausalLM_KVCOMP(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModel_KVCOMP(config)
        self.vocab_size = config.vocab_size
