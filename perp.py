...
import gc
from models.llama_kivi_ablated import LlamaForCausalLM_KVCOMP_ABLATED
...
@torch.no_grad()
def test_ppl_hf(model, tokenizer, datasets=['wikitext2'], ppl_seqlen=2048):
    results = {}
    ...
    for dataset in datasets:
        ...
        if "c4" in dataset:
            ...
            testenc = testloader.input_ids
        if testenc.dim() == 2 and testenc.size(0) == 1:
            testenc = testenc[0]
        ...
        for i in tqdm(range(nsamples)):
            batch = testenc[(i * seqlen):((i + 1) * seqlen)].unsqueeze(0).to(model.device)
            outputs = model(batch)
...
def benchmark_llama():
    ...
    benchmark_results_llama = test_ppl_hf(model, tokenizer)
...
def benchmark_kvcomp():
    ...
    model = LlamaForCausalLM_KVCOMP_ABLATED.from_pretrained(
        pretrained_model_name_or_path=model_name_or_path,
        config=config,
        cache_dir=CACHE_DIR,
        torch_dtype=torch.float16,
    ...
    benchmark_results_ours = test_ppl_hf(model, tokenizer)
